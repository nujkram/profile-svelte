<script lang="ts">
	...
	const jsonLd = JSON.stringify({
		...
		url: origin,
		image: `${origin}/profile-image.jpg`,
	...
	// Concatenated so neither the Svelte nor ESLint parser sees a literal closing script tag
	...
	$effect(() => {
		// Skip all entrance/scroll animations for users who prefer reduced motion;
		// content simply renders in place (counters jump straight to their totals).
		if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
			factElements.forEach((el) => (el.textContent = el.dataset.target || '0'));
			return;
		}
		...
		import('$lib/utils/gsap').then(({ gsap, ScrollTrigger, animateCounter }) => {
			...
			ctx = gsap.context(() => {
				...
				heroTl
					...
					.from('.hero-name', { y: 30, opacity: 0, duration: 0.6, ease: 'power3.out' }, '-=0.4')
					.from('.hero-title', { y: 20, opacity: 0, duration: 0.5, ease: 'power3.out' }, '-=0.3')
					.from(
						'.hero-social > a',
						{
							y: 15,
		...
		<div class="relative pl-8 border-l-2 border-primary-500/30">
			{#each [...(profile?.experiences || [])].reverse() as exp}
				<div class="experience-item relative mb-8 last:mb-0">
					...
					<div
						class="card card-ghost p-4 hover:bg-primary-500/5 hover:border-primary-500/40 transition-all duration-300"
					>
	...
	<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
		<section id="education" bind:this={educationSection} class="card p-6">
			...
			<div class="space-y-6">
				<div>
					...
					<p class="italic text-primary-500 dark:text-primary-400 underline">
						{profile?.mastersSchool || 'NA'}
					</p>
		...
		<section id="skills" bind:this={skillsSection} class="card p-6">
			...
			<p class="mb-4 opacity-70">
				It takes talent to make money, but it takes brains to keep them.
			</p>
